...
AstNode parse_token(int *index, Ast *ast, char *include_dir, Tokens tokens)
{
    AstNode node = {0};
    ...
    case TOKEN_LPAREN:
    {
        ...
        if (!strcmp(node.call.name, "include"))
        {
            ...
            if (!fp)
            {
                ...
                memcpy(final_str, include_dir, strlen(include_dir));

                if (include_dir[strlen(include_dir) - 1] != '/')
                {
                    strcat(final_str, "/");
                }

                strcat(final_str, tokens.data[*index]._string);

                fp = fopen(final_str, "r");
            ...
            {
                vec_push(ast, parse_token(&i, ast, include_dir, new_tokens));
            }
        ...
        else if (!strcmp(node.call.name, "defmacro"))
        {
            ...
            *index += 2;
            ...
            {
                vec_push(&ret, parse_token(index, ast, include_dir, tokens));
            }
        ...
        else
        {
            ...
            while (tokens.data[++*index].type != TOKEN_RPAREN)
            {
                vec_push(&node.call.params, parse_token(index, ast, include_dir, tokens).value);
            }
        }
...
Ast parse(Tokens tokens, char *include_dir)
{
    ...
    {
        AstNode curr = parse_token(&i, &ast, include_dir, tokens);
